...
_PATTERNS: tuple[tuple[str, str, str], ...] = (
    ...
    (CREDENTIAL, "labelled credential",
     r"(?i)\bcredential\b\s+(?:is\s+)?[\"']?([A-Za-z0-9][A-Za-z0-9._~+/=-]{11,})"),
    (CREDENTIAL, "spoken assigned secret",
     r"(?i)\b(api[_-]?key|secret|password|passwd|token|client[_-]?secret)\b\s+is\s+[\"']?([^\s\"',;]{8,})"),
