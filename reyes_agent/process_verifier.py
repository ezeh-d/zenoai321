"""Independent post-condition checks for consequential actions (OS process/file).

NAMING / COEXISTENCE
--------------------
This is the OS-level "did the app actually launch / does the file exist" verifier
(``Verdict`` + ``verify(action, args, result)`` + ``app_is_running``) used by the
remote desktop executor to corroborate open_app. It lives here, separate from
Codex's strategy/evidence-ledger ``action_verifier.py``, so the two verifiers
coexist without either overwriting the other. Both are kept.
