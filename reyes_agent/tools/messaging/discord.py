"""Discord desktop, driven the same way Slack is.
...
Discord's quick switcher is Ctrl+K, same idea as Slack's.
...
LAUNCH = "cmd /c start \"\" discord://"
SEARCH_KEYS = ('ctrl', 'k')

SIGNED_OUT_MARKERS = ("log in", "login to discord", "welcome back!", "need an account?")
OFFLINE_MARKERS = ("checking for updates", "connecting", "no internet connection", "reconnecting")
...
def check_state(handle: int, trace: desktop.Trace) -> str:
    ...
    if any(marker in text for marker in SIGNED_OUT_MARKERS):
        trace.add("check_login", False, "Discord is showing a sign-in screen",
                  (time.perf_counter() - started) * 1000)
        return models.AUTH_REQUIRED
    if any(marker in text for marker in OFFLINE_MARKERS):
        trace.add("check_online", False, "Discord reports it is not connected",
                  (time.perf_counter() - started) * 1000)
