"""WhatsApp desktop, driven the same way Slack is.
...
WhatsApp Desktop focuses the chat search with Ctrl+F.
...
LAUNCH = "cmd /c start \"\" whatsapp://"
SEARCH_KEYS = ('ctrl', 'f')

SIGNED_OUT_MARKERS = ("scan the qr code", "link a device", "log into whatsapp", "steps to log in")
OFFLINE_MARKERS = ("computer not connected", "trying to reconnect", "phone not connected", "syncing messages")
...
def check_state(handle: int, trace: desktop.Trace) -> str:
    ...
    if any(marker in text for marker in SIGNED_OUT_MARKERS):
        trace.add("check_login", False, "WhatsApp is showing a sign-in screen",
                  (time.perf_counter() - started) * 1000)
        return models.AUTH_REQUIRED
    if any(marker in text for marker in OFFLINE_MARKERS):
        trace.add("check_online", False, "WhatsApp reports it is not connected",
                  (time.perf_counter() - started) * 1000)
